        ...
        <span class="font-medium">Admin</span>
            ...
            <main class="p-6">
                ...
                <div class="bg-cyan-800 text-white p-3 text-center rounded mb-4 shadow-md w-full">
                    <h2 class="text-2xl font-semibold">DATA KRS MAHASISWA POLITEKNIK NEGERI CILACAP</h2>
                </div>
                ...
                <!-- Main Content -->
                  <div class="mt-6 bg-white p-6 rounded-lg shadow-md max-w-5xl mx-auto">
                    <div class="flex justify-between items-center mt-4">
                      <a href="/admin/tambahKRS" class="bg-green-700 hover:bg-green-600 text-white px-4 py-2 rounded transition"">
                            Tambah
                        </a> 
                    ...
                    <table id="krsTable" class="min-w-full border border-gray-300 text-sm text-center">
                <thead class="bg-cyan-500 text-gray-800">
                        ...
                        <tbody>
                            ...
                            <tr class="hover:bg-gray-100">
                                ...
                                <a href="/admin/editKRS/{{ $kr['id_krs'] }}" class="text-gray-500 border border-transparent hover:border-blue-600 hover:text-blue-600 hover:scale-110 transition duration-200 ease-in-out">✏</a>
                                <form action="{{ url('admin/hapusKRS/' . $kr['id_krs']) }}" method="POST" class="inline">
                                  @csrf
                                  @method('DELETE')
                                    <button type="button" onclick="confirmDelete({{ $kr['id_krs'] }})" 
                                    class="text-gray-500 border border-transparent hover:border-red-600 hover:text-red-600 hover:scale-110 transition duration-200 ease-in-out">🗑</button>                                </form>
                                </td>
                            </tr> 
                            @endforeach
                    </table>
                    <script>
              function confirmDelete(id_krs) {
                Swal.fire({
              title: 'Yakin ingin menghapus?',
              text: "Data KRS Mahasiswa akan dihapus secara permanen!",
              icon: 'warning',
            ...
            .then((result) => {
                  if (result.isConfirmed) {
                    ...
                    form.method = 'POST';
                    form.action = `/admin/hapusKRS/${id_krs}`;
